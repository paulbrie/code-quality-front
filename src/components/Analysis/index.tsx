...
const Analysis = () => {
  ...
  return (
    <div className={styles["analysisComponent"]}>
      ...
      <div>
        Total: <span style={{ color: "red" }}>errors</span>: {errorCount} |{" "}
        <span style={{ color: "yellow" }}>warnings</span>: {warningCount}
      </div>
      <br />
      <div>
        {errorCount !== 0 &&
          messages.map((m, index) => (
            <div>
              <div className={styles["errorMessage"]}>
                Error {index + 1}: {m.message}
                <div>
                  Lines: {m.line} - {m.endLine}, Columns: {m.column} -{" "}
                  {m.endColumn}, ruleId: {m.ruleId}
                </div>
                <br />
              </div>
            </div>
